/*
  ...
  for example, 4 is 1 (I) less than 5 (V): IV and 9 is 1 (I) less 
  ...
  at most 3 times to represent multiples of 10. You cannot append 
  5 (V), 50 (L), or 500 (D) multiple times. If you need to append 
  ...
  Note: 49 is not 1 (I) less of 50 (L) because the conversion is 
...
function intToRoman(num){
    ...
    while(num  > 0){
        ...
        switch(digit){
            case 4: 
                result.unshift(fives[multiplier]);
                result.unshift(ones[multiplier]);
                num = num - digit;
                num = num / 10;
                multiplier++;
                break;
            case 9:
                result.unshift(ones[multiplier+1]);
                result.unshift(ones[multiplier]);
                num = num - digit;
                num = num / 10;
                multiplier++;
